...
const saveMutation = trpc.platformConnections.saveApiKey.useMutation({
  onSuccess: () => {
    toast.success(`Configuration de ${platform} sauvegardée`);
    utils.platformConnections.getStatus.invalidate();
    onClose();
  },
  onError: (error) => {
...
const handleSave = () => {
  ...
  saveMutation.mutate({
    platform,
    apiKey,
    config: selectedModel ? { model: selectedModel } : undefined,
  });
};
